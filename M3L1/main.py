...
app = Flask(__name__)
frases_lista =['La mayoría de las personas que sufren adicción tecnológica experimentan un fuerte estrés cuando se encuentran fuera del área de cobertura de la red o no pueden utilizar sus dispositivos'
...
contra = ['x','c','d','x','v','b','g','h','j','k','l','m','n','p','q','r','s','t','u','v','w','x','y','z','1','2','3','4','5','6','7','8','9','0','@','#','$','%','&','*','(',')','+','-','_','=']

@app.route("/datorandom")
def datorand():
    return f'<p>{random.choice(frases_lista)}</p>'

@app.route("/")
def hello_world():
    return '''<h1>Hello, World!</h1>
    <a href=/datorandom> ira una frasea leatoria </a>
    <a href=/gencon> Genera una contraseña segura </a>'''

@app.route("/gencon")
def generadorcon():
    return f'<p> {(random.choice(contra) for _ in range(12))} </p>'
